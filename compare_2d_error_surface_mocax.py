"""
2D Error Surface Visualization: MoCaX Approximation

Creates 3D surface plots showing how MoCaX approximation errors vary
...
def build_mocax_2d(domain, fixed_params, n_nodes=11):
    """
    Build 2D MoCaX approximation V(K, T).
    ...
    Returns:
        mocax_obj: MoCaX approximation object
        build_time: Time to build approximation (seconds)
    """
    print("\n" + "="*70)
    print(f"Building 2D MoCaX Approximation: V(K, T) with {n_nodes}×{n_nodes} nodes")
    print("="*70)
...
def evaluate_mocax_2d(mocax_obj, samples, ground_truth_prices, ground_truth_thetas):
    """
    Evaluate MoCaX 2D approximation on random samples.
    ...
    # Derivative IDs for MoCaX
    deriv_price = mocax_obj.get_derivative_id([0, 0])  # V(K, T)
    deriv_theta = mocax_obj.get_derivative_id([0, 1])  # ∂V/∂T

    print("\n" + "="*70)
    print("Evaluating MoCaX on Random Samples")
    print("="*70)

    start = time.time()

    for i in tqdm(range(n_samples), desc="Evaluating MoCaX", ncols=80):
...
def plot_error_surface_3d(K_grid, T_grid, errors_grid, error_type, domain, filename):
    """
    Create 3D surface plot of errors using regular grid.

    Args:
        K_grid: 2D meshgrid array of K values
        T_grid: 2D meshgrid array of T values
        errors_grid: 2D array of percentage errors
        error_type: String describing error type (e.g., "Price", "Theta")
        domain: List of (min, max) for [K, T]
    ...
    # Create smooth surface using regular grid
    surf = ax.plot_surface(K_grid, T_grid, errors_grid, cmap='viridis',
    ...
    ax.set_zlabel(f'{error_type} Error (%)', fontsize=12, labelpad=10)
    ax.set_title(f'MoCaX 2D Approximation: {error_type} Error Surface\n'
                f'Domain: K=[{domain[0][0]}, {domain[0][1]}], T=[{domain[1][0]}, {domain[1][1]}]',
                fontsize=14, pad=20)

    # Add colorbar
    cbar = fig.colorbar(surf, ax=ax, shrink=0.5, aspect=5, pad=0.1)
    cbar.set_label(f'{error_type} Error (%)', fontsize=11)
...
def plot_error_subplots(K_grid, T_grid, price_errors_grid, theta_errors_grid,
    ...
    # Overall title
    fig.suptitle(f'MoCaX 2D Approximation Error Analysis: {n_nodes}×{n_nodes} Nodes\n'
                f'Domain: K=[{domain[0][0]}, {domain[0][1]}], T=[{domain[1][0]}, {domain[1][1]}]',
                fontsize=15, y=0.98)
...
def plot_error_convergence(results, domain, filename):
    ...
    # Overall title
    fig.suptitle(f'MoCaX 2D Approximation: Error Convergence Analysis\n'
                f'Domain: K=[{domain[0][0]}, {domain[0][1]}], T=[{domain[1][0]}, {domain[1][1]}]',
                fontsize=14, y=0.99)
...
def main():
    ...
    print("="*80)
    print("2D ERROR SURFACE VISUALIZATION: MoCaX Approximation")
    print("Multiple Chebyshev Node Configurations")
    print("="*80)
    print("\nExperiment Configuration:")
    print("  Method: MoCaX 2D approximation V(K, T)")
    print("  Sampling: Regular grid (50×50)")
    ...
    print("  Output: 4 figures with 2 subplots each")
    ...
    n_points_per_dim = 50  # 50x50 = 2500 grid points
    print(f"\n{'='*70}")
    print(f"Generating Grid Samples ({n_points_per_dim}x{n_points_per_dim})")
    print("="*70)
    ...
    for n_nodes in node_configs:
        ...
        mocax_obj, build_time = build_mocax_2d(domain, fixed_params, n_nodes=n_nodes)

        # Evaluate MoCaX
        price_errors, theta_errors, eval_time = evaluate_mocax_2d(
            mocax_obj, samples, ground_truth_prices, ground_truth_thetas
        )
        ...
        filename = f"mocax_2d_error_n{n_nodes}.png"
    ...
    print("="*80)
    plot_error_convergence(results, domain, "mocax_2d_convergence.png")
    ...
    for n_nodes in node_configs:
        print(f"  • mocax_2d_error_n{n_nodes}.png (price + theta error surfaces)")
    print(f"\nGenerated convergence plot:")
    print(f"  • mocax_2d_convergence.png (exponential error decay)")
    print("\nKey Observations:")
    print("  • Lower nodes (4, 6) show visible approximation errors")
    print("  • Higher nodes (8, 12) demonstrate improved accuracy")
    print("  • Convergence plot shows exponential error reduction (spectral convergence)")
